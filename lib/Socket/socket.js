...
export const makeSocket = (config) => {
    ...
    const newsletterFunctions = makeNewsletterSocket(config)(sock);
    return {
        type: 'md',
        ws,
        ev,
        ...
        const msg = sendButtonsMessage(jid, content, options);
        return sock.sendMessage(jid, msg, options);
    },
    
    // Message interactif
    sendInteractiveMessage: async (jid, content, options) => {
        const msg = await sendInteractiveMessage(jid, content);
    ...
    // Liste
    sendListMessage: (jid, content, options) => {
        const msg = sendListMessage(jid, content);
        return sock.sendMessage(jid, msg, options);
    },
    
    // Carrousel
    sendCarouselMessage: async (jid, content, options) => {
        const msg = sendCarouselMessage(jid, content);
        return sock.relayMessage(msg.key.remoteJid, msg.message, { 
            messageId: msg.key.id,
            ...options
        });
